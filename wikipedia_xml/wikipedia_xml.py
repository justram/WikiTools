            ...
            features=datasets.Features(
                {
                    ...
                    "infobox_media": datasets.Sequence(feature=datasets.Value("string")),
...
def _parse_and_clean_wikicode(raw_content, parser, language):
    ...
    magic_patterns = re.compile("__[A-Z]*__", flags=re.UNICODE)

    media_prefixes = "|".join(["File", "Image", "Media"] + MEDIA_ALIASES.get(language, []))
    media_patterns = re.compile(f"^(?:{media_prefixes}):", flags=re.IGNORECASE | re.UNICODE)

    cat_prefixes = "|".join(["Category"] + CAT_ALIASES.get(language, []))
    cat_patterns = re.compile(f"^(?:{cat_prefixes}):", flags=re.IGNORECASE | re.UNICODE)

    infobox_prefixes = 'infobox'
    infobox_patterns = re.compile(f"^(?:{infobox_prefixes})", flags=re.IGNORECASE | re.UNICODE)
    ...
    def rm_span(obj):
        return str(obj.tag) in {"span"}

    def remove_prefix(obj, pattern):
        try:
            text = obj.strip()
    ...
    for template in wikicode.filter_templates():
        if infobox_patterns.match(template.name.strip()):
            for param in template.params:
                value = param.value.strip()
                # only procss media files started with [File:], [Image:], etc
                if media_patterns.match(value):
                    media_file = re.sub(media_patterns, "", value)
                    info_media.append(media_file)
    
    # Filter individual sections to clean.
    for s_idx, section in enumerate(wikicode.get_sections(flat=True, include_lead=True, include_headings=True)):
        # parse headings
        if s_idx == 0:
            section_title.append('Introduction')
        else:
            heading = section.filter_headings()[0]
            
            # clean section title
            _title = heading.title
            for tag in _title.ifilter_tags(matches=rm_span, recursive=True):
                try_remove_obj(tag, _title)

            section_title.append(_title.strip_code().strip())
        ...
        for link in section.ifilter_wikilinks(recursive=True):
            ...
            if media_patterns.match(str(link_title)):
                media_text = remove_prefix(link_title, media_patterns)
                media.append(media_text)
        ...
        # remove useless childs
        for tag in section.ifilter_tags(matches=rm_tag, recursive=True):
            try_remove_obj(tag, section)
            
        # plaintext
        plain_text = re.sub(magic_patterns, "", section.strip_code().strip())
